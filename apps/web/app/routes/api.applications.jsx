...
async function proxyApplicationsRequest(request, method) {
  const apiBase = process.env.SERVICE_API_BASE_URL || DEFAULT_API_BASE;
  const session = await getSession(request);
  const token = session.get('token');
  const currentUrl = new URL(request.url);
  const targetUrl = `${apiBase}/api/applications${currentUrl.search}`;
...
export async function loader({ request }) {
  ...
  try {
    return await proxyApplicationsRequest(request, 'GET');
  } catch (error) {
    return json(
      buildErrorPayload(error?.payload, error?.message || 'Unable to proxy applications request.'),
...
export async function action({ request }) {
  ...
  try {
    return await proxyApplicationsRequest(request, 'POST');
  } catch (error) {
    return json(
      buildErrorPayload(error?.payload, error?.message || 'Unable to proxy applications request.'),
      { status: error?.status || 500 },
